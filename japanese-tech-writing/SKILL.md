...
- 例の節の中でも、**その節の問い・帰結に関係しない**過剰な詳細だけを省く。議論に必要な具体は残す。省略の典型は、ログや報告に含まれる議論に効かない精度（時刻、ステータスコード、比率など）や、後で参照しない固有名である。
...
- 章や節で定式化・術語（その文書で導入した記号や概念名）を導入したら、以後はその語で通す。「文脈」「ツール」「AI」のような曖昧語に後退しない（定式化する前の導入語として「文脈」などを使うのはよい）。
...
読み物として緩急を付ける場合の例外は `cognitive-rhythm-writing` が定める。本規範だけで書くときは、上記と「演出の抑制」の抑制を守る。
...
論点を増やさず「ちゃんと書いている感」だけを付ける言い回し（「重要なのは〜である」「正面から扱う」「多角的に」「掘り下げる」「非常に」）を使わない。その文書で定義した術語を議論に使うのはよい。空虚な装飾として使うのが問題である。
推敲時は [references/llm-phrases.md](references/llm-phrases.md) の一覧で点検する。
