...
description: >-
  設計文書・調査報告・対策案・命名（状態名、条件名、型名、メソッド名）を書く前に、語より先に指示対象と役割を対応表で固定する。
  新しい概念や名前を導入する文書を書くときに使う。引用・機械的な編集・既存名の再利用・定型出力には使わない。
---
...
規律の基準は、本文より先に対応表を独立成果物として保存し、語より先に指示対象と役割を固定することである。
...
### 1. 対応表を独立成果物として先に保存する

本文を書く前に、対応表を独立ファイルとして保存する。

- 保存先: 作業ディレクトリの `referent-table-<slug>.md` (成果物が `output/` 配下なら
  同じディレクトリに置く)。完成文書の先頭に表を置くだけでは、語より先に対象を
  固定する規律にならない。
...
- 対応表を提出せず本文を書き始めたことに気づいたら、そこで止めて対応表を独立ファイルに
  書き、本文の中心語彙を表に合わせて修正する。表と食い違う箇所だけを書き直せばよい。
