...
4. 終了コード 2（Guard によるフォールバック）または 3（接続失敗。3 秒の疎通確認で判定）のときは原文が出力されるので、下書きをそのまま使う。**再試行や自前の修正はしない**（Guard の判断を尊重する）

## 強制モード（`--force`）

ユーザーが **明示的に**「Guard を無視して書き直して」「強制で」「フォールバックせずに出して」と指示したときだけ、`--force` を付けて実行する。サービスは Guard の検査を行ったうえで、不合格でも書き直し本文を返す（応答の `guard.bypassed: true`、標準エラーに `BYPASSED` と検査理由）。

- ユーザーの指示がない限り `--force` は付けない。フォールバックしたから自動で強制に切り替える、はしない
- 強制で得た本文は意味変更の検知を放棄しているので、**数値・否定・条件・コードを原文と照合してから**最終回答にする
- サーバー側で強制が許可されていない場合（`force_denied`）は通常どおり原文が返る
