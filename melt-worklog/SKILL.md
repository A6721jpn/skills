...
5. 検証してNotion貼り付け用HTML・Markdown・構造化タスク・根拠ファイルを保存する。

   ```powershell
   & '<Python>' '<Skill>/scripts/report.py' render --bundle '<作業用ディレクトリ>' --tasks '<tasks.json>' --out '<現在の作業ディレクトリ>/outputs/worklog'
   ```

   出力は日時付きで旧版を残す。日報は「見出し・本文・出典」の単純な構造にし、HTMLコメント、taskマーカー、インデント付き本文、行末の強制改行用スペースを入れない。内部IDと見出しの対応はJSONに保存する。同じ見出しになる作業は統合するか、実際の対象・工程が分かる題名に直す。
   最終回答では `*-Notion貼付用.html` を優先して案内する。ブラウザーで開いて「日報をコピー」を押すと、見出し・段落・リンクを含む書式付き本文をクリップボードへ渡す。コピー権限が使えない場合は本文が選択されるのでCtrl+Cを使う。Notionの本文へ通常のCtrl+Vで貼り付ける。Markdownをチャットに示す場合はコードブロックに入れず、余分なバックスラッシュやHTML実体参照に変換しない。長い元ログをチャットに貼らない。
   Slackは元のHTTPSリンクを保持する。元URLがないログの出典は「ローカル」と明記する。ローカルファイルはNotionへの貼り付けだけでは参照できないため、その制約を伝える。Notionで開けるURLが必要なら、別途許可された保存先に出典を配置してから差し替える。URLを捏造したり、ログを公開GitHubへ載せたりしない。この出力変更だけでNotionページを作成・変更しない。
   ...
   **生成後に日報を編集した場合も、渡す前に必ず再検査する。** 現行形式はJSONの作業見出しと本文を対応付けて検査する。貼り付け用HTMLの本文・出典リンク・プレーンテキストがMarkdownと一致することも検査する。変更は原則tasks.jsonから再生成し、MarkdownとHTMLを同時に更新する。旧マーカー形式の日報も再検査できるが、新規出力には使わない。
