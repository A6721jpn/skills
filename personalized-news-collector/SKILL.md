...
- The coordinator alone may write the digest and seen state. Subagents never receive writable paths.
...
- When model selection is exposed and the user requests the quality-first route, prefer `gpt-5.6-luna` with `max` reasoning for independent workers. Stop at evidence convergence and hard safety limits; do not create unbounded duplicate work.
